...
export default function APVouchersPage() {
  ...
  const fetchData = async (params?: any) => {
    setLoading(true)
    try {
      const res = await accountingApi.apVouchers.list(params)
  ...
  const columns = [
    ...
    { title: '应付金额', dataIndex: 'amount', key: 'amount', render: (v: string, record: any) => formatMoney(v, record.currency) },
    { title: '账期', dataIndex: 'period', key: 'period' },
    { title: '操作', key: 'action', render: (_: any, record: any) => (
      <Button type="link" onClick={() => navigate(`/fin/accounting/ap-vouchers/${record.voucher_id}`)}>详情</Button>
    )},
  ]
