...
from app.application.prod.verifiers.v04_redis_cache_protection_verifier import RedisCacheProtectionVerifier
from app.domain.prod.engine.enums import VerificationConclusion, VerificationItem


class TestRedisCacheProtectionVerifier:
    def setup_method(self):
        self.verifier = RedisCacheProtectionVerifier()

    def test_item_is_correct(self):
        assert self.verifier.item == VerificationItem.CACHE
    ...
    def test_report_contains_verification_item(self):
        config = make_config(parameters={})
        result = asyncio.run(self.verifier.execute(config))
        assert result.report["verification_item"] == "V04_CACHE"
