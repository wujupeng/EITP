...
from app.application.prod.verifiers.v06_saga_compensation_verifier import SagaCompensationVerifier
from app.domain.prod.engine.enums import VerificationConclusion, VerificationItem


class TestSagaCompensationVerifier:
    def setup_method(self):
        self.verifier = SagaCompensationVerifier()

    def test_item_is_correct(self):
        assert self.verifier.item == VerificationItem.SAGA
    ...
    def test_report_contains_verification_item(self):
        config = make_config(parameters={})
        result = asyncio.run(self.verifier.execute(config))
        assert result.report["verification_item"] == "V06_SAGA"
