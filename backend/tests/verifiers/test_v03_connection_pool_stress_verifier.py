...
from app.application.prod.verifiers.v03_connection_pool_stress_verifier import ConnectionPoolStressVerifier
from app.domain.prod.engine.enums import VerificationConclusion, VerificationItem


class TestConnectionPoolStressVerifier:
    def setup_method(self):
        self.verifier = ConnectionPoolStressVerifier()

    def test_item_is_correct(self):
        assert self.verifier.item == VerificationItem.CONNPOOL
    ...
    def test_report_contains_verification_item(self):
        config = make_config(parameters={})
        result = asyncio.run(self.verifier.execute(config))
        assert result.report["verification_item"] == "V03_CONNPOOL"
